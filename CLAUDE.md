...
Dummy Loader is a small Go HTTP server that exposes an API for generating CPU and memory load, used to experiment with resource management config in Docker/Kubernetes. Module: `github.com/sha1n/dummy-loader`, Go 1.25.
...
- `make lint` — `gofmt -d server` (diff-only; this is the only linting, no golangci-lint config exists)
...
- `make lint` runs `golangci-lint` (config: `.golangci.yml`). As of this writing it reports ~25 pre-existing findings (mostly `errcheck` unchecked returns and `revive` naming/comment conventions) that predate the lint setup — don't treat those as caused by your changes; only fix lint issues you introduce unless asked to clean up the backlog.
